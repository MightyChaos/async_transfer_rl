...
class GameACNetwork(object):
    ...
    def prepare_loss(self, entropy_beta):
        with tf.device(self._device):
            ...
            policy_loss1 = - tf.reduce_sum(
                tf.reduce_sum(tf.multiply(log_pi1, self.a1), reduction_indices=1) * self.td1 + entropy1 * entropy_beta)
            policy_loss2 = - tf.reduce_sum(
                tf.reduce_sum(tf.multiply(log_pi2, self.a2), reduction_indices=1) * self.td2 + entropy2 * entropy_beta)
    ...
    def _fc_factorized_variable(self, weight_shape):
        input_channels = weight_shape[0]
        output_channels = weight_shape[1]
        d = 1.0 / np.sqrt(input_channels)
        bias_shape = [output_channels]
        ...
        bias_2 = tf.Variable(tf.random_uniform(bias_shape, minval=-d, maxval=d))
        return weight_A, z_1, z_2, weight_B, bias_1, bias_2
...
class GameACFFNetwork(GameACNetwork):
    def __init__(self,
        ...
        with tf.device(self._device), tf.variable_scope(scope_name) as scope:
            ...
            self.W_fc1_A, self.z_1, self.z_2, self.W_fc1_B, self.b_fc1_1, self.b_fc1_2 = self._fc_factorized_variable([2592, 256])
            ...
            # policy (output)
            self.pi1 = tf.nn.softmax(tf.matmul(h_fc1_1, self.W_fc2_1) + self.b_fc2_1)
            self.pi2 = tf.nn.softmax(tf.matmul(h_fc1_2, self.W_fc2_2) + self.b_fc2_2)
            # value (output)
            v_1 = tf.matmul(h_fc1_1, self.W_fc3_1) + self.b_fc3_1
            v_2 = tf.matmul(h_fc1_2, self.W_fc3_2) + self.b_fc3_2
